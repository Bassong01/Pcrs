...
import { IconOverview, IconCases, IconPersons, IconRecords, IconAlerts, IconUsers, IconAudit, IconFolderOpen, IconTarget, IconClock, IconPerson, IconLock, IconEye, IconEyeOff } from './icons.jsx'
...
const navItems = [
    ['overview', 'navOverview', IconOverview], ['cases', 'navCases', IconCases], ['persons', 'navPersons', IconPersons],
    ['records', 'navRecords', IconRecords], ['alerts', 'navAlerts', IconAlerts],
]
const roleHome = { admin: 'overview', police_officer: 'cases', judicial_authority: 'alerts', general_inspectorate: 'audit' }
// Pages each role may open. Keep in sync with the authorize() guards on the backend GET routes.
const roleViews = {
    admin: ['overview', 'cases', 'persons', 'records', 'alerts', 'users', 'audit'],
    police_officer: ['overview', 'cases', 'persons', 'records', 'alerts'],
    judicial_authority: ['overview', 'alerts', 'records', 'persons'],
    general_inspectorate: ['overview', 'users', 'audit'],
}
...
function App() {
    ...
    useEffect(() => {
        if (!session) return
        if (!(roleViews[session.user.role] || []).includes(activeView)) { setActiveView(roleHome[session.user.role] || 'overview'); return }
        loadView(activeView, wantedOnly)
    }, [session, activeView, offlineMode, wantedOnly])
    ...
    const allowedViews = roleViews[session?.user.role] || []
    const canView = (view) => allowedViews.includes(view)
        ...
        try {
            ...
            if (offlineMode) {
                ...
                if (form.type === 'alert') {
                    ...
                    setActiveView('alerts'); setNotice('Alert saved locally for judicial review. It will not appear as wanted until authorized and synced.')
                    await loadView('alerts', true)
                } else {
            ...
            if (form.type === 'alert') {
                ...
                storeNotification(notification); setNotifications(getStoredNotifications()); setShowNotifications(false)
                setActiveView('alerts'); setNotice('Alert submitted for judicial review. It will not appear as wanted until authorized.')
                await loadView('alerts', true)
            } else {
    ...
    function changeView(view) { if (!canView(view)) return; setSearch(''); setNotice(''); setWantedOnly(false); setActiveView(view) }
    ...
    return <div className="app-shell">
        <aside className="sidebar"><div className="brand"><img src={logo} alt="" className="brand-mark" /><span><strong>{t('brandName')}</strong><small>{t('brandSub')}</small></span></div><p className="eyebrow">{t('operationsConsole')}</p><nav>{navItems.filter(([value]) => canView(value)).map(([value, key, Icon]) => <button className={activeView === value ? 'nav-item active' : 'nav-item'} key={value} onClick={() => changeView(value)}><span className="nav-icon"><Icon /></span>{t(key)}</button>)}{(canView('users') || canView('audit')) && <><p className="nav-section">{t('governance')}</p>{canView('users') && <button className={activeView === 'users' ? 'nav-item active' : 'nav-item'} onClick={() => changeView('users')}><span className="nav-icon"><IconUsers /></span>{t('navUsers')}</button>}{canView('audit') && <button className={activeView === 'audit' ? 'nav-item active' : 'nav-item'} onClick={() => changeView('audit')}><span className="nav-icon"><IconAudit /></span>{t('navAudit')}</button>}</>}</nav><div className="sidebar-note"><span className="status-dot" />{t('secureNetwork')}<br /><small>{t('regionalDelegations')}</small></div></aside>
        <main className="main-content"><header className="topbar"><div><p className="eyebrow">{new Date().toLocaleDateString(dateLocale, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</p><h1>{title}</h1></div><div className="profile"><button className="lang-toggle" onClick={toggleLang} title="Switch language">{lang === 'en' ? 'FR' : 'EN'}</button><div className="notification-wrap"><button className="notification-button" aria-label="Notifications" onClick={() => setShowNotifications((visible) => !visible)}>⌁{notifications.length > 0 && <span className="notification-count">{notifications.length > 9 ? '9+' : notifications.length}</span>}</button>{showNotifications && <NotificationPanel notifications={notifications} onClear={() => setNotifications([])} t={t} />}</div><span className="avatar">{session.user.first_name[0]}{session.user.last_name[0]}</span><span><strong>{session.user.first_name} {session.user.last_name}</strong><small>{session.user.role.replaceAll('_', ' ')}</small></span><button className="logout" onClick={logout}>{t('signOut')}</button></div></header>{error && <div className="error-banner">{error}<button onClick={() => setError('')}>{t('dismiss')}</button></div>}{notice && <div className="notice-banner">{notice}</div>}{loading ? <div className="loading">{t('loadingData')}</div> : activeView === 'overview' ? <Overview dashboard={dashboard} setActiveView={changeView} canView={canView} t={t} /> : activeView === 'alerts' ? <Directory view={activeView} rows={rows} search={search} setSearch={setSearch} onSearch={() => loadView('alerts')} canWrite={canWrite} canReviewAlerts={canReviewAlerts} onCreate={canWrite ? () => setForm(newForm('alert', session.user)) : undefined} onAction={alertAction} t={t} lang={lang} /> : activeView === 'users' || activeView === 'audit' ? <Directory view={activeView} rows={rows} search={search} setSearch={setSearch} onSearch={() => loadView(activeView)} t={t} lang={lang} /> : activeView === 'persons' ? <Directory view={activeView} rows={rows} search={search} setSearch={setSearch} onSearch={() => loadView(activeView)} canWrite={canWrite} onCreate={canWrite ? () => setForm(newForm('person', session.user)) : undefined} onOpenPerson={openPersonProfile} t={t} lang={lang} /> : <Directory view={activeView} rows={rows} search={search} setSearch={setSearch} onSearch={() => loadView(activeView)} canWrite={canWrite} onCreate={canWrite ? () => setForm(newForm(activeView === 'cases' ? 'case' : 'record', session.user)) : undefined} t={t} lang={lang} />}</main>{form && <FormModal form={form} setForm={setForm} submitForm={submitForm} loading={loading} t={t} lang={lang} />}{selectedPerson && <PersonProfilePanel person={selectedPerson} onClose={() => setSelectedPerson(null)} t={t} />}</div>
}
...
function Login({ credentials, setCredentials, onSubmit, onOfflineLogin, loading, error, t, lang, toggleLang }) {
    ...
    return <main className="login-page"><button type="button" className="lang-toggle login-lang-toggle" onClick={toggleLang}>{lang === 'en' ? 'FR' : 'EN'}</button><section className="login-art"><p className="eyebrow">{t('loginKicker')}</p><h1>{t('loginHeading1')}<br /><em>{t('loginHeading2')}</em></h1><p>{t('loginPitch')}</p><div className="login-lines"><span>{t('loginStep1')}</span><span>{t('loginStep2')}</span><span>{t('loginStep3')}</span></div></section><form className="login-form" onSubmit={onSubmit}><div className="brand"><img src={logo} alt="" className="brand-mark" /><span><strong>{t('brandName')}</strong><small>{t('brandSub')}</small></span></div><h2>{t('welcomeBack')}</h2><p className="muted">{t('loginSubtitle')}</p><label className="field-pill"><span className="field-icon"><IconPerson /></span><input type="text" placeholder={t('username')} value={credentials.email} onChange={(event) => setCredentials({ ...credentials, email: event.target.value })} /></label><label className="field-pill"><span className="field-icon"><IconLock /></span><input type={showPassword ? 'text' : 'password'} placeholder={t('password')} value={credentials.password} onChange={(event) => setCredentials({ ...credentials, password: event.target.value })} /><button type="button" className="field-toggle" onClick={() => setShowPassword((visible) => !visible)} aria-label={showPassword ? t('hidePassword') : t('showPassword')}>{showPassword ? <IconEyeOff /> : <IconEye />}</button></label>{error && <p className="form-error">{error}</p>}<button className="primary-button" disabled={loading}>{loading ? t('authenticating') : t('accessConsole')}</button><button type="button" className="outline-button" onClick={onOfflineLogin}>{t('useOffline')}</button><div className="demo-access"><span>{t('demoAccess')}</span>{demoAccounts.map((account) => <button type="button" key={account.email} onClick={() => setCredentials(account)}>{account.label}</button>)}</div></form></main>
}
...
function Overview({ dashboard, setActiveView, canView, t }) {
